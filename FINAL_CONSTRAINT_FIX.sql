-- NUCLEAR OPTION - DISABLE ALL CONSTRAINTS TEMPORARILY (SYNTAX FIXED)
...
-- Show some sample IDs for each user type (alternative approach)
SELECT 
    p.user_type,
    p.id as sample_id,
...
WHERE p.id IN (
    SELECT p2.id 
    FROM profiles p2 
    WHERE p2.user_type = p.user_type 
    ORDER BY p2.created_at 
    LIMIT 3
)
ORDER BY p.user_type, p.created_at;
