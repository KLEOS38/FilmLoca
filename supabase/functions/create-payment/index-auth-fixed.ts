  ...
  try {
    ...
    if (!propertyId || !startDate || !endDate || !totalPrice) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Missing required fields: propertyId, startDate, endDate, totalPrice",
          code: "MISSING_FIELDS",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (start < minStartTime) {
      console.error("Booking too soon:", { start, now: minStartTime });
      return new Response(
        JSON.stringify({
          error: "Booking must be made at least 1 hour in advance",
